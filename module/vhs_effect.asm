...
VHS_MAX_SHIFT   EQU     16              ; max horizontal tear in pixels
...
.planes_ok:
    ...
    ; ── Allocate clean source buffer (copy of your screen, never overwritten) ──
    move.l  #VHS_SCREEN_SIZE,d0
    move.l  #MEMF_CHIP|MEMF_CLEAR,d1
    jsr     LVO_ALLOCMEM(a6)
    tst.l   d0
    beq     .fail
    move.l  d0,vhs_BufSrc
    ...
    move.l  vhs_BufA,VV_DRAWBUF(a5)
    move.l  vhs_BufB,VV_DISPBUF(a5)
    move.l  vhs_BufSrc,VV_SRCBUF(a5)
    move.l  vhs_NoisePlane,VV_NOISEBUF(a5)
    move.l  vhs_CopA,VV_COPLIST(a5)
    move.l  vhs_CopB,VV_COPLIST2(a5)
    move.l  vhs_YourCopList,VV_YOURCOP(a5)
    ...
    ; ── Copy your screen into clean source buffer ──
...
.pal_loop:
    ...
    ; R_vhs: lerp toward grey at 75%: grey + (R-grey)/4
    move.w  d1,d5
    sub.w   d4,d5
    asr.w   #2,d5
...
VHS_DoFrame:
    ...
    ; Effect active — run full pipeline
    bsr     vhs_FillNoise
    bsr     vhs_BuildShiftTable
    bsr     vhs_DoVertRoll
    bsr     vhs_DoNoiseOverlay
    bsr     vhs_DoChromaSmear
    bsr     vhs_BuildEffectCopList
    bsr     vhs_ApplyVHSPalette
    bra     .swap_and_install

.do_normal:
    ; Effect fully stopped — rebuild clean copper in both buffers, restore
    ; Rebuild both copper lists so neither holds stale per-line entries
    bsr     vhs_BuildNormalCopList
    move.l  VV_COPLIST(a5),d0
    move.l  VV_COPLIST2(a5),d1
    move.l  d1,VV_COPLIST(a5)
    move.l  d0,VV_COPLIST2(a5)
    bsr     vhs_BuildNormalCopList

    ; Restore hardware palette
    bsr     vhs_ApplyNormalPalette

    ; Restore YOUR copper list to the hardware
...
.swap_and_install:
    ...
    move.l  VV_DRAWBUF(a5),d0
    move.l  VV_DISPBUF(a5),d1
    move.l  d1,VV_DRAWBUF(a5)
    move.l  d0,VV_DISPBUF(a5)
    ...
    ; Install effect copper list (high word first — OCS requirement)
    move.l  VV_COPLIST(a5),d0
    swap    d0
    move.w  d0,CUSTOM+COP1LCH
    swap    d0
...
.chk_fast:
    ...
    ; RWIN_FAST: full effect, advance vertical roll
...
vhs_BuildNormalCopList:
    ...
    move.l  VV_SRCBUF(a5),a1            ; display from clean source
...
.bpl_ptr:
    ...
    swap    d0
    move.w  d0,(a0)+                ; low word (after swap, original low is back)
